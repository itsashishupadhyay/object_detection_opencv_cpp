#!/usr/bin/env python3
"""
Expansion auto-labeler for Cassini ISSNA imagery (Round 3).

Labels new images from the expansion set while PRESERVING existing
approved round-2 labels. Same sanity-check thresholds as round 2.

Output:
- New YOLO label files in data/cassini_issna_yolo/labels/train/<image_id>.txt
- New images symlinked/copied into data/cassini_issna_yolo/images/train/
- Review bundle in review/cassini_issna/auto_labels_round3/
- Stats JSON at artifacts/cassini_issna/auto_label_expansion_stats.json
"""
...
REVIEW_DIR = WORKSPACE / "review" / "cassini_issna" / "auto_labels_round3"
...
# Round 2 thresholds (preserved exactly)
MIN_DIAMETER_PX = 15
...
# (user explicitly requested this for expansion)
...
def main():
    print("=" * 60)
    print("ICES Expansion Auto-Labeler: cassini / issna  (ROUND 3)")
    print("=" * 60)
    ...
    # New images = in manifest but not in train or test
    new_ids = set(manifest.keys()) - train_ids - test_ids

    # Eligible for labeling: (train OR new) AND NOT dropped AND NOT test
    # AND NOT already labeled
    eligible_ids = (train_ids | new_ids) - dropped - test_ids - existing_labels
    print(f"New images (not in any split): {len(new_ids)}")
    ...
    for i, image_id in enumerate(eligible_sorted):
        ...
        # Body subtends > 95% of frame -> skip (strict whole-frame-bbox rule)
        ...
        # Whole-frame bbox check (round 2 threshold)
    ...
    print(f"\n{'='*60}")
    print(f"Expansion labeling complete:")
    print(f"  New labels generated: {stats['new_accepted']}")
    print(f"  Preserved round-2 labels: {len(existing_labels)}")
    ...
    # Read existing labels to get their body info
    ...
    stats_path = ARTIFACTS / "cassini_issna" / "auto_label_expansion_stats.json"
    ...
    # ============================================================
    print(f"\n{'='*60}")
    print("Generating human review bundle (round 3 expansion)...")
    ...
    easy_sample = sample_up_to(easy, 10)
    hard_sample = sample_up_to(hard, 10)
    ambiguous_sample = sample_up_to(ambiguous, 10)
    ...
    random_sample = sample_up_to(random_pool, 10) if random_pool else []
    ...
    with open(decision_path, "w") as f:
        f.write("# Decision: Auto-Labels Round 3 (Expansion) -- cassini / issna\n\n")
        f.write("STATUS: \n\n")
    ...
    with open(review_request_path, "w") as f:
        f.write("# Review Request: Auto-Labels for cassini / issna  (ROUND 3 -- EXPANSION)\n\n")
        f.write("This is the expansion labeling round. The 288 labels from round 2 are preserved.\n")
        f.write("New labels were generated for 750 expansion images plus previously-unlabeled\n")
        f.write("train-split images, using the same round-2 thresholds.\n\n")
        f.write("**Iteration log:** `research_log/0013_auto_label_expansion.md`\n\n")

        f.write("## Summary\n\n")
        f.write(f"- **New labels generated (this round):** {stats['new_accepted']}\n")
        f.write(f"- **Preserved round-2 labels:** {len(existing_labels)}\n")
        ...
        f.write("\n## Review Buckets\n\n")
        f.write(f"Four buckets of sample images (10 each) in `review/cassini_issna/auto_labels_round3/`:\n\n")
        f.write(f"1. **easy/** ({len(easy_sample)} images) -- Large, well-centered, high confidence.\n")
        f.write(f"2. **hard/** ({len(hard_sample)} images) -- Small or off-center.\n")
        f.write(f"3. **ambiguous/** ({len(ambiguous_sample)} images) -- Low confidence.\n")
        ...
        f.write("`review/cassini_issna/auto_labels_round3/DECISION.md`\n\n")
